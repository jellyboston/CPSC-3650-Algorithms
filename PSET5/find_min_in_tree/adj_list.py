...
class AdjListGraph:
    """ An graph with vertices labelled by consecutive integer indices starting from 0 and
        with a value associated with each vertex.
    """
    # TODO: make this abstract
    def __init__(self, n, v):
        if len(v) != n:
            raise ValueError(f"values must have length equal to number of vertices {n}; got {len(v)}")
        self._size = n
        self._values = v[:]
    ...
    def value(self, u):
        """ Returns the value of vertex u in this graph.
